...
7. Run `python3 scripts/build_dataset.py`.
8. Run `python3 -m pytest`.
9. Review `data/processed/release_status.csv`, `data/processed/calibration_summary.csv`, and `data/processed/data_quality_report.csv` before looking at any market output.
10. Review `data/processed/model_output.json`, `data/processed/audit_report.md`, and `data/processed/source_manifest.csv`.
11. Re-execute `notebooks/tx_gop_senate_runoff_model.ipynb` only if you need supporting exploratory analysis.
12. Review `data/processed/wager_value_table.csv`, `data/processed/poll_diagnostics.csv`, `data/processed/poll_miss_diagnostics.csv`, `data/processed/runoff_county_projection.csv`, `data/processed/sensitivity.csv`, and `data/processed/shock_model.csv`.
